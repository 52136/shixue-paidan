...
if option == "?? 派单员填单":
    ...
    with st.form("submit_form"):
        ...
        songxin_shuliang = st.number_input("送心数量（??）", min_value=1, step=1)
        ...
        if submitted:
            if paidan_ren.strip() == "":
                ...
                st.success(f"? {paidan_ren} 的单子已提交！??{songxin_shuliang}，提成￥{ticheng_jine}")
            else:
                st.error("数量和金额必须大于0")
...
# ========== 管理员统计 ==========
    ...
    if df.empty:
        ...
        for _, row in summary.iterrows():
            st.write(f"**{row['派单员']}**：??{row['总送心数量']} 颗，￥{row['总提成金额']}，共 {row['订单数']} 单")
    ...
    if delete_option == "删除全部记录":
        ...
        if paidanren_list:
            ...
            if st.button(f"?? 确认删除 {selected} 的所有记录"):
                df_del = load_data()
